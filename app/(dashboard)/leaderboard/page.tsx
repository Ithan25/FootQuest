    ...
    <div className="space-y-10 pb-20">
      <section className="text-center">
        <h1 className="flex items-center justify-center gap-2 text-3xl font-black tracking-tight text-white sm:text-4xl">
          <Trophy className="h-8 w-8 text-[#FFE600] drop-shadow-md" /> Le Classement
        </h1>
      ...
      <section className="flex items-end justify-center gap-2 pt-10 sm:gap-6">
        ...
        {second && (
          <div className="flex w-[30%] max-w-[140px] flex-col items-center">
            ...
            <div className="relative mt-2 flex w-full flex-col items-center justify-start rounded-t-2xl bg-gradient-to-t from-zinc-700/40 to-zinc-800/20 px-2 py-3 shadow-lg sm:mt-3 sm:p-4 h-[100px] sm:h-[130px] border-t-4 border-zinc-500">
              ...
              <span className="text-[10px] font-semibold text-[#FFE600] mt-0.5 sm:text-xs sm:mt-1">{second.foot_points} FP</span>
            </div>
          </div>
        )}
        ...
        {first && (
          <div className="z-10 flex w-[34%] max-w-[160px] flex-col items-center">
            <Crown className="mb-1 h-6 w-6 animate-bounce text-[#FFE600] drop-shadow-[0_0_8px_rgba(255,230,0,0.8)] sm:mb-2 sm:h-8 sm:w-8" />
            <PodiumAvatar player={first} rank={1} size="lg" color="amber" />
            <div className="relative mt-2 flex w-full flex-col items-center justify-start rounded-t-2xl bg-gradient-to-t from-[#FFE600]/15 to-amber-900/10 px-2 py-3 shadow-2xl sm:mt-3 sm:p-4 h-[130px] sm:h-[170px] border-t-4 border-[#FFE600]">
              {/* Neon Base Effect */}
              <div className="absolute inset-x-0 bottom-0 h-1 bg-[#FFE600]/60 blur-sm" />
              <div className="absolute inset-x-2 bottom-0 h-[2px] bg-[#FFE600] shadow-[0_0_15px_rgba(255,230,0,0.8)]" />
              
              <span className="text-2xl font-black text-[#FFE600] drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)] sm:text-3xl">1</span>
              ...
              <span className="text-xs font-bold text-[#FFE600] mt-0.5 sm:text-sm sm:mt-1">{first.foot_points} FP</span>
            </div>
          </div>
        )}
        ...
        {third && (
          <div className="flex w-[30%] max-w-[140px] flex-col items-center">
            ...
            <div className="relative mt-2 flex w-full flex-col items-center justify-start rounded-t-2xl bg-gradient-to-t from-orange-900/30 to-orange-950/10 px-2 py-3 shadow-lg sm:mt-3 sm:p-4 h-[90px] sm:h-[110px] border-t-4 border-orange-600">
              ...
              <span className="text-[10px] font-semibold text-[#FFE600] mt-0.5 sm:text-xs sm:mt-1">{third.foot_points} FP</span>
            </div>
          </div>
        )}
      </section>
      ...
      {theRest.length > 0 && (
        <section className="mx-auto w-full max-w-2xl rounded-2xl border border-zinc-800 bg-zinc-900 p-1 sm:p-2">
          {theRest.map((player) => (
      ...
      {currentUserRank && (!theRest.find(p => p.id === currentUserRank.id) && !podium.find(p => p.id === currentUserRank.id)) && (
        <div className="fixed bottom-20 left-0 right-0 z-30 mx-auto w-full max-w-2xl px-4 animate-in fade-in slide-in-from-bottom-8">
          <div className="rounded-2xl border-2 border-[#00FF87]/30 bg-[#09090b]/95 p-3 shadow-2xl backdrop-blur-md">
...
function PodiumAvatar({ player, rank, size, color }: { player: UserRank; rank: number; size: "md" | "lg", color: "amber" | "slate" | "orange" }) {
  ...
  const borderColors = {
    amber: "border-[#FFE600] shadow-[#FFE600]/50",
    slate: "border-zinc-500 shadow-zinc-400/30",
    orange: "border-orange-500 shadow-orange-500/20",
  };

  return (
    <div className={cn("relative rounded-full shadow-xl bg-zinc-800 flex items-center justify-center overflow-hidden", sizeClasses, borderColors[color])}>
      {player.avatar_url ? (
...
function LeaderboardRow({ player, isCurrentUser, hideBorder = false }: { player: UserRank; isCurrentUser: boolean; hideBorder?: boolean }) {
  return (
    <div className={cn(
      ...
      !hideBorder && "border-b border-zinc-800/50 last:border-0 rounded-none",
      isCurrentUser && "bg-[#00FF87]/10"
    )}>
      <div className="flex items-center gap-4">
        ...
        {/* Avatar */}
        <div className="flex h-10 w-10 overflow-hidden rounded-full bg-zinc-800 shrink-0">
          {player.avatar_url ? (
            <img src={player.avatar_url} alt={player.pseudo} className="h-full w-full object-cover" />
          ) : (
      ...
      {/* Points */}
      <div className="font-bold text-[#FFE600] tabular-nums">
        {player.foot_points} <span className="text-xs text-[#FFE600]/50 uppercase">FP</span>
      </div>
    </div>
  );
}
