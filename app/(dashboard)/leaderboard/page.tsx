...
export default async function LeaderboardPage() {
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      {/* Podium Section */}
      <section className="flex flex-col sm:flex-row items-end justify-center gap-4 sm:gap-6 pt-10">
        {/* 2ND PLACE */}
        {second && (
          <div className="order-2 flex flex-col items-center sm:order-1 flex-1 max-w-[140px]">
            <PodiumAvatar player={second} rank={2} size="md" color="slate" />
            <div className="relative mt-3 flex w-full flex-col items-center justify-start rounded-t-2xl bg-gradient-to-t from-slate-200 to-slate-100 dark:from-slate-800 dark:to-slate-700/50 p-4 shadow-lg h-[130px] border-t-4 border-slate-300 dark:border-slate-500">
              <span className="text-xl font-bold text-slate-700 dark:text-slate-300">2</span>
              <span className="mt-1 font-bold truncate w-full text-center text-slate-800 dark:text-white">{second.pseudo}</span>
              <span className="text-xs font-semibold text-amber-600 dark:text-amber-400 mt-1">{second.foot_points} FP</span>
            </div>
          </div>
        )}
        ...
        {/* 1ST PLACE */}
        {first && (
          <div className="order-1 flex flex-col items-center sm:order-2 flex-1 max-w-[160px] z-10">
            <Crown className="mb-2 h-8 w-8 animate-bounce text-amber-400 drop-shadow-md" />
            <PodiumAvatar player={first} rank={1} size="lg" color="amber" />
            <div className="relative mt-3 flex w-full flex-col items-center justify-start rounded-t-2xl bg-gradient-to-t from-amber-200/40 to-amber-100 dark:from-amber-900/40 dark:to-amber-800/20 p-4 shadow-xl h-[170px] border-t-4 border-amber-400 shadow-amber-500/20">
              <span className="text-3xl font-black text-amber-600 dark:text-amber-400 drop-shadow-sm">1</span>
              <span className="mt-1 text-lg font-black truncate w-full text-center text-slate-900 dark:text-white">{first.pseudo}</span>
              <span className="text-sm font-bold text-amber-600 dark:text-amber-400 mt-1">{first.foot_points} FP</span>
            </div>
          </div>
        )}
        ...
        {/* 3RD PLACE */}
        {third && (
          <div className="order-3 flex flex-col items-center sm:order-3 flex-1 max-w-[140px]">
            <PodiumAvatar player={third} rank={3} size="md" color="orange" />
            <div className="relative mt-3 flex w-full flex-col items-center justify-start rounded-t-2xl bg-gradient-to-t from-orange-200/50 to-orange-100/50 dark:from-orange-950/40 dark:to-orange-900/20 p-4 shadow-lg h-[110px] border-t-4 border-orange-400/70 dark:border-orange-700">
              <span className="text-xl font-bold text-orange-700 dark:text-orange-500">3</span>
              <span className="mt-1 font-bold truncate w-full text-center text-slate-800 dark:text-white">{third.pseudo}</span>
              <span className="text-xs font-semibold text-amber-600 dark:text-amber-400 mt-1">{third.foot_points} FP</span>
            </div>
          </div>
        )}
      </section>
